...
<div class="cart bundled_item_cart_content" data-title="<?php echo esc_attr( $bundled_item->get_title() ); ?>" data-product_title="<?php echo esc_attr( $bundled_item->get_product()->get_title() ); ?>" data-visible="<?php echo $bundled_item->is_visible() ? 'yes' : 'no'; ?>" data-optional_suffix="<?php echo esc_attr( $bundled_item->get_optional_suffix() ); ?>" data-optional="<?php echo $bundled_item->is_optional() ? 'yes' : 'no'; ?>" data-type="<?php echo $bundled_product->get_type(); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $bundled_product_variations ) ); ?>" data-bundled_item_id="<?php echo $bundled_item->get_id(); ?>" data-custom_data="<?php echo esc_attr( json_encode( $custom_product_data ) ); ?>" data-product_id="<?php echo $bundled_product_id; ?>" data-bundle_id="<?php echo $bundle_id; ?>" <?php echo $bundled_item->is_optional() && ! $bundled_item->is_optional_checked() ? 'style="display:none"' : ''; ?>>
	<table class="variations 1222" cellspacing="0">
		<tbody>
<!-- <input type="number" id="quantity_5fd20c12c42c6" class="input-text qty bundled_qty text" step="1" min="1" max="10" name="bundle_quantity_3" value="1" title="Qty" size="4" placeholder="" inputmode="numeric">
 -->			<?php
	// 		echo "<pre>";
 // print_r($bundled_item);
		...
		/*	foreach ($varientData as $key => $value) { ?>
					Name : <?php echo $value['attributes']['attribute_flawer']; ?>
<input type="number" id="quantity_5fd207866164e" class="input-text qty bundled_qty text" step="1" min="1" max="5" name="bundle_quantity_5" value="1" title="Qty" size="4" placeholder="" inputmode="numeric">
			<?php 
			...
			foreach ( $bundled_product_attributes as $attribute_name => $options ) {
				...
				?><tr class="attribute_options <?php echo $is_attribute_value_configurable ? 'attribute_value_configurable' : 'attribute_value_static' ; ?>" data-attribute_label="<?php echo esc_attr( wc_attribute_label( $attribute_name ) ); ?>" id="<?php echo $bundled_item->get_product()->get_id(); ?>">
					<td class="label">
