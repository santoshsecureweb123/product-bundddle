...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	 ...
	 * @since 2.4.0
	 */
	function wc_dropdown_variation_attribute_options( $args = array(),$bundl_id,$varient_id  ) {
		// print_r($args);exit;
...
//  MANPREET
if ( ! function_exists( 'wc_multipe_qty_variation_attribute_options' ) ) {
	 ...
	 * @since 2.4.0
	 */
	function wc_multipe_qty_variation_attribute_options( $args = array(),$bundl_id,$varient_id ) {
		...
		// echo"THERE";print_r($bundl_id);exit;
		// echo"THERE";print_r($args['bundled_item']->data->bundled_item_id);exit;
		// Get selected value.
		if ( false === $args['selected'] && $args['attribute'] && $args['product'] instanceof WC_Product ) {
			$selected_key = 'attribute_' . sanitize_title( $args['attribute'] );
			// phpcs:disable WordPress.Security.NonceVerification.Recommended
			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options, true ) ) {
						...
						$html .= '<div class= "box-custom-wrapper"> <span>'.$term->name.'</span> <input type="number" id="quantity_'.$bundl_id.'_'.$term->slug.'_'.$varient_id[$term->slug].'" class="input-text qty bundled_qty text" step="1" min="1" max="10" name="bundle_quantity_'.$bundl_id.'_'.$term->slug.'_'.$varient_id[$term->slug].'" value="1" title="Qty" size="4" placeholder="" inputmode="numeric"></div>';
					}
				}
			} else {
				foreach ( $options as $option ) {
					...
					$html .= '<div class= "box-custom-wrapper"> <span>'.$option.'</span>  <input type="number" id="quantity_'.$bundl_id.'_'.$option.'" class="input-text qty bundled_qty text" step="1" min="1" max="10" name="bundle_quantity_'.$bundl_id.'_'.$option.'" value="1" title="Qty" size="4" placeholder="" inputmode="numeric"></div>';
